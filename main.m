...
Impact_timing = 0.4;             % [s]: Timing of impact
Impact_step_number = 10;         
...
while 1
    ...
    elseif Foot_state ==  1 % LF swing
        LF = footTrajectory(t_step, step_phase, number_of_step, Foot_state, L_or_R, T_step, p_total, LF_prev, RF_prev);
        RF = RF_prev;
        contact = RF_prev;
    elseif Foot_state == -1 % RF swing
        LF = LF_prev;
        RF = footTrajectory(t_step, step_phase, number_of_step, Foot_state, L_or_R, T_step, p_total, LF_prev, RF_prev);
        contact = LF_prev;
    end
...
% Debug
temp_p_stored_1 = p_ref_stored(:, 1:600);
temp_p_stored_2 = p_ref_stored(:, 601:1200);
temp_p_stored_3 = p_ref_stored(:, 1201:1800);
temp_p_stored_4 = p_ref_stored(:, 1801:2400);
...
temp_etaL_stored_4 = etaL_stored(1, 1801:2400);
...
temp_etaR_stored_4 = etaR_stored(1, 1801:2400);
...
temp_contact_stored_4 = contact_stored(:, 1801:2400);
...
temp_contact_ref_stored_4 = contact_ref_stored(:, 1801:2400);
...
temp_delcontact_stored_4 = delcontact_stored(:, 1801:2400);
...
subplot(3,4,1);
plot(1:i, delcontact_stored(1,1:i), 'b-', 'LineWidth', 1.5); hold on;
plot(1:i, 0.6 * ones(1,i),   'r--', 'LineWidth', 1);        % 한보폭
plot(1:i, 0.3 * ones(1,i),   'r--', 'LineWidth', 1);        % 반보폭
grid on;
ax = gca;         
ax.XTick = 0:600:i;
xlim([1 i]);
xlabel('Step index');
ylabel('delcontact_x');
legend('delcontact_x','Reference step length', 'Reference half');
title('deltac_x');


%––– Fig 2: delcontact_stored(2,:) + reference
subplot(3,4,5);
plot(1:i, delcontact_stored(2,1:i), 'b-', 'LineWidth', 1.5); hold on;
plot(1:i, 0.22 * ones(1,i),      'r--', 'LineWidth', 1);
plot(1:i, -0.22 * ones(1,i),      'r--', 'LineWidth', 1);
grid on;
ax = gca;         
ax.XTick = 0:600:i;
...
xlabel('Step index');
ylabel('delcontact_y');
legend('delcontact_y','Reference left', 'Reference right');
title('deltac_y');

%––– Fig 1: wx
subplot(3,4,2);
plot(1:i, w_stored(1,1:i),       'b-',  'LineWidth', 1.5); hold on;
plot(1:i, w_ref_stored(1,1:i),   'r--', 'LineWidth', 1);
grid on;
ax = gca;         
ax.XTick = 0:600:i;
xlim([1 i]);
...
%––– Fig 2: wy
subplot(3,4,6);
plot(1:i, w_stored(2,1:i),       'b-',  'LineWidth', 1.5); hold on;
plot(1:i, w_ref_stored(2,1:i),   'r--', 'LineWidth', 1);
grid on;
ax = gca;         
ax.XTick = 0:600:i;
xlim([1 i]);
...
%––– Fig 3: wz
subplot(3,4,10);
plot(1:i, w_stored(3,1:i),       'b-',  'LineWidth', 1.5); hold on;
plot(1:i, w_ref_stored(3,1:i),   'r--', 'LineWidth', 1);
grid on;
ax = gca;         
ax.XTick = 0:600:i;
xlim([1 i]);
...
%––– subplot(2,2,2): footstep location x
subplot(3,4,3);
h1 = plot(1:i, p_next_foot_step_stored(1,1:i), 'b:',  'LineWidth',1.5);  hold on;
h2 = plot(1:i, p_ref_stored(1,1:i),             'k-.', 'LineWidth',1.5); % dash-dot
h3 = plot(1:i, p_stored(1,1:i),                 'r-',  'LineWidth',1.5); % dotted
h4 = plot(1:i, COM_stored(1,1:i),                 'g-',  'LineWidth',1.5);
h5 = plot(1:i, COM_ref_stored(1,1:i),             'm-',  'LineWidth',1.5);
grid on;
ax = gca;         
ax.XTick = 0:600:i;
ax.YTick = 0:0.3:2.4;
...
xlabel('Step index');
ylabel('x');
legend([h1,h2,h3,h4,h5],{'p\_next_x','p\_ref_x','p_x', 'COM\_x', 'COM\_Ref_x'},'Location','northwest');
title('footstep location x');

%––– subplot(2,2,4): footstep location y
subplot(3,4,7);
h1 = plot(1:i, p_next_foot_step_stored(2,1:i), 'b:',  'LineWidth',1.5);  hold on;
h2 = plot(1:i, p_ref_stored(2,1:i),             'k-.', 'LineWidth',1.5);
h3 = plot(1:i, p_stored(2,1:i),                 'r-',  'LineWidth',1.5);
h4 = plot(1:i, COM_stored(2,1:i),                 'g-',  'LineWidth',1.5);
h5 = plot(1:i, COM_ref_stored(2,1:i),             'm-',  'LineWidth',1.5);
grid on;
ax = gca;         
ax.YTick = -0.33:0.11:0.33;
xlim([1 i]);
ylim([-0.3 0.3]);
xlabel('Step index');
ylabel('y');
legend([h1,h2,h3,h4,h5],{'p\_next_y','p\_ref_y','p_y', 'COM\_y', 'COM\_Ref_y'},'Location','northwest');
title('footstep location y');

%-- LF/RF x-t
subplot(3,4,4);
h1 = plot(1:i, LF_stored(1,1:i), 'b-',  'LineWidth', 1.5); hold on;
h2 = plot(1:i, RF_stored(1,1:i), 'r-',  'LineWidth', 1.5);
h3 = plot(1:i, p_ref_stored(1,1:i),  'k--', 'LineWidth', 1.5);
grid on;
ax = gca;         
ax.XTick = 0:600:i;        
ax.YTick = 0:0.3:2.4;
...
xlabel('Step index');
ylabel('LF/RF x-t');
legend([h1,h2,h3],{'LF_x','RF_x','C_ref'},'Location','northwest');
title('LF/RF x-t');

%-- LF/RF y-t
subplot(3,4,8);
h1 = plot(1:i, LF_stored(2,1:i), 'b-',  'LineWidth', 1.5); hold on;
h2 = plot(1:i, RF_stored(2,1:i), 'r-',  'LineWidth', 1.5);
h3 = plot(1:i, p_ref_stored(2,1:i),  'k--', 'LineWidth', 1.5);
grid on;
ax = gca;         
ax.XTick = 0:600:i;   
ax.YTick = -0.33:0.11:0.33;
...
xlabel('Step index');
ylabel('LF/RF y-t');
legend([h1,h2,h3],{'LF_y','RF_y','C_ref'},'Location','northwest');
title('LF/RF y-t');
...
%%
% figure; plot(LF_stored(1,1:i-1), LF_stored(2,1:i-1)); grid on;
% xlabel('x [m]'); ylabel('y [m]'); title('Left Foot Trajectory (XY Plane)');
% xlim([min(LF_stored(1,1:i-1)) max(LF_stored(1,1:i-1))]);
% ylim([min(LF_stored(2,1:i-1)) max(LF_stored(2,1:i-1))]);

% figure; plot(1:i, LF_stored(1,1:i)); grid on;
% xlabel('steptime [s]'); ylabel('x [m]'); title('Left Foot Trajectory (X-t Plane)');
% xlim([1 i]);
% ylim([min(LF_stored(1,1:i)) max(LF_stored(1,1:i))]);
