...
deg = 0;                                    % [deg]: Degree of Impact force (0: to Right / 90: to Front / 180: to Left / 270: to Back)
...
while 1
    ...
    [theta_ref_horizon, COM_ref_horizon, w_ref_horizon, dCOM_ref_horizon, rL_ref_horizon, rR_ref_horizon, rE_ref_horizon, etaL_ref_horizon, etaR_ref_horizon, etaE_ref_horizon, f_EXT_ref_horizon] = ...
    ...
    [mL, fL, mR, fR] =  nextState(x0, ...
                                  ...
                                  rL_ref_horizon, rR_ref_horizon, rE_ref_horizon, f_EXT_ref_horizon, etaL_ref_horizon, etaR_ref_horizon, etaE_ref_horizon);
    ...
    t_span = [0 PARA.dt];
    y0 = [theta; COM; w; dCOM];
    [t_ode, y_ode] = ode45(@(t_ode, y_ode) odefunc(y_ode, contact_wrench_result, Foot_state, PARA.m_all, PARA.I, PARA.g, rL, rR, rE, flag_EXT, f_EXT), t_span, y0);  
...
if flag_VISUALIZATION == 1
    ...
    % Xlim0 = [min(Xall)-pad, max(Xall)+pad];
    % Ylim0 = [min(Yall)-pad, max(Yall)+pad];
end
%---
...
varsToSave = {'t_stored', 'T_step', ...
              'step_length', 'step_width', 'Impact_t_idx', 'number_of_step',...
              'p_stored', 'p_ref_stored', 'COM_stored', 'COM_ref_stored', ...
              'theta_stored', 'i', 'm_obj', ...
              'LF_stored', 'RF_stored', ...
