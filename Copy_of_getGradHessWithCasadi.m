...
contact     = SX.sym('contact'    , delcontact_length * 1, 1);
delcontact  = SX.sym('delcontact' , delcontact_length * 1, 1);
contact_ref = SX.sym('contact_ref', delcontact_length * 1, 1);

v = [X; U; delcontact];

W_Q = SX.sym('W_Q', H * state_length, 1);
W_R = SX.sym('W_R', H * input_length, 1);
W_C = SX.sym('W_C', 1 * delcontact_length, 1);
...
for i = 1:H
    ...
    k1 = A *  x_k + B * u_k + d + Ac * delcontact;
    k2 = A * (x_k + 0.5 * dT * k1) + B * u_k + d + Ac * delcontact;
    k3 = A * (x_k + 0.5 * dT * k2) + B * u_k + d + Ac * delcontact;
    k4 = A * (x_k + dT * k3) + B * u_k + d + Ac * delcontact;
