...
h1 = plot(t_stored(1:i-1), delcontact_stored(1,1:i-1),        'b-',  'LineWidth', 2.0); hold on;
...
legend(subplot1, [h1,h2,h3], {'\deltac_x','Stride^{Ref}', 'Half\_Stride^{Ref}'}, 'Location','best');
title(subplot1, '\deltac_x');

%––– Fig 2: delcontact_stored(2,:) & reference
subplot2 = nexttile(getTile(2, 1));
h1 = plot(t_stored(1:i-1), delcontact_stored(2,1:i-1),                           'b-', 'LineWidth', 2.0); hold on;
...
legend(subplot2, [h1,h2,h3], {'\deltac_y','LF^{Ref}', 'RF^{Ref}'}, 'Location','best');
...
h4 = plot(t_stored(1:i-1), COM_stored(1,1:i-1),              'g-',  'LineWidth', 1.5);
...
ylabel(subplot3, 'x (m)');
legend(subplot3, [h1,h2,h3,h4,h5], {'c_x','c^{Ref}_x','c^{Next}_x','COM_x','COM^{Ref}_x'}, 'Location','best');
...
h4 = plot(t_stored(1:i-1), COM_stored(2,1:i-1),              'g-',  'LineWidth', 1.5);
...
ylabel(subplot4, 'y (m)');
legend(subplot4, [h1,h2,h3,h4,h5], {'c_y','c^{Ref}_y','c^{Next}_y','COM_y','COM^{Ref}_y'}, 'Location','best');
title(subplot4, 'Footstep Location Y');


%––– Fig 5: LF_x, RF_x
subplot5 = nexttile(getTile(1, 3)); 
h1 = plot(t_stored(1:i-1), LF_stored(1,1:i-1),    '-',  'Color', color_LF, 'LineWidth', 2.0); hold on;
h2 = plot(t_stored(1:i-1), RF_stored(1,1:i-1),    '-',  'Color', color_RF, 'LineWidth', 2.0); 
...
ylabel(subplot5, 'x (m)');
legend(subplot5, [h1,h2,h3], {'LF_x','RF_x','c^{Ref}_x'}, 'Location','best');
title(subplot5, 'LF_x & RF_x');

%––– Fig 6: LF_y, RF_y
subplot6 = nexttile(getTile(2, 3)); 
h1 = plot(t_stored(1:i-1), LF_stored(2,1:i-1),    '-',  'Color', color_LF, 'LineWidth', 2.0); hold on;
h2 = plot(t_stored(1:i-1), RF_stored(2,1:i-1),    '-',  'Color', color_RF, 'LineWidth', 2.0); 
...
ylabel(subplot6, 'y (m)');
legend(subplot6, [h1,h2,h3], {'LF_y','RF_y','c^{Ref}_y'}, 'Location','best');
...
%        'OuterPosition',[ (1-0.9)/2, (1-0.9*9/16)/2, 0.9, 0.9*9/16 ], ...
%        'Name','dtheta');
% 
% ––– Fig 1: wx
% subplot(1,3,1);
